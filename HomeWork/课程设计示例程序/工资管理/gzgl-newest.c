#include <conio.h>  /*屏幕操作函数库 !coin.h 不在标准库中*/
...
void stringinput(char *t, unsigned int lens, const char *notice)
{
  char n[255];
  do
  {
...
void Sort(ZGGZ tp[], int n)
{
  ...
  for (i = 0; i < n; i++)
  {
    ...
    if (flag == 0)
      break; /*若标记flag=0，意味着没有交换了，排序已经完成*/
  }
